...
test('真实 v14 Tool 事件迁移后经 DAO、HTTP 与 SSE 只公开稳定调用标识', {
  ...
  const migrationPool = createPool(migrationDatabaseUrl!)
  const suffix = crypto.randomUUID()
  const analysisId = `v14-http-analysis-${suffix}`
  const sessionId = `v14-http-session-${suffix}`
  const executionId = `v14-http-execution-${suffix}`
  const projectionId = `v14-http-projection-${suffix}`
  const batchId = `v14-http-batch-${suffix}`
  const toolCallId = `v14-http-call-${suffix}`
  await migrate(migrationDatabaseUrl!)
  try {
    await migrationPool.query(
      `INSERT INTO analyses (id, symbol, status, active, created_at, updated_at)
       VALUES ($1, 'V14HTTP', 'completed', false, now(), now())`, [analysisId],
    )
    await migrationPool.query(
      `INSERT INTO agent_sessions (id, analysis_id, is_primary, execution_id, status,
         latest_sequence, created_at, updated_at)
       VALUES ($1, $2, true, $3, 'completed', 2, now(), now())`,
      [sessionId, analysisId, executionId],
    )
    await migrationPool.query(
      `INSERT INTO agent_executions (id, session_id, generation, status, terminal, created_at, updated_at)
    ...
    await migrationPool.query(
      `INSERT INTO tool_projection_versions (id, execution_id, version, role, stage, schema_hash,
       ...
       VALUES ($1, $2, 1, 'main', 'research', 'v14-http-hash',
         '[]', '["tool_not_available"]', '{}', now())`, [projectionId, executionId],
    )
    await migrationPool.query(
    ...
    await migrationPool.query(
      `INSERT INTO tool_batch_calls (batch_id, tool_call_id, tool_name, position, status,
       ...
       VALUES ($1, $2, 'tool_not_available', 1, 'failed', now(), now(), 1,
         '{"result":{"error":"tool_not_available"},"isError":true}')`, [batchId, toolCallId],
    )
    await migrationPool.query(
      `INSERT INTO agent_events (session_id, sequence, operation_id, payload_json, created_at) VALUES
       ($1, 1, $2, '{"type":"tool_call","name":"legacy_hidden_tool","input":{"secret":"v14-must-not-leak"}}', now()),
       ($1, 2, $3, '{"type":"tool_result","name":"legacy_hidden_tool","result":{"error":"tool_not_available"},"isError":true}', now())`,
    ...
    try {
      const runtime = await fetch(
        `${baseUrl}/api/agent-sessions/${sessionId}/tool-runtime?executionId=${executionId}`,
      ).then((response) => response.json())
      const research = await fetch(`${baseUrl}/api/research/${analysisId}`)
        .then((response) => response.json())
      const response = await fetch(`${baseUrl}/api/agent-sessions/${sessionId}/events`)
      ...
      const sse = await readThroughReader(reader, 'event: tool_result')
      await reader.cancel()
      const publicReadback = JSON.stringify({ runtime, research, sse })
      assert.doesNotMatch(publicReadback, /legacy_hidden_tool|v14-must-not-leak/)
      assert.match(publicReadback, new RegExp(toolCallId))
      assert.deepEqual(research.trace.map((entry: { type: string; name: string; toolCallId: string }) => ({
        type: entry.type, name: entry.name, toolCallId: entry.toolCallId,
      })), [
        { type: 'tool_call', name: 'tool_not_available', toolCallId },
        { type: 'tool_result', name: 'tool_not_available', toolCallId },
      ])
      assert.match(sse, /event: tool_call/)
      assert.match(sse, /event: tool_result/)
    } finally {
