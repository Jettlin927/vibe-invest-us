...
- 研究自由对话 Thread：可按需调用金融工具、创建受控 subagent、长上下文 compaction、停止/恢复和 SSE 回放；
- 在自由对话中检索历史报告和讨论、引用具体报告版本，并继续补查；
- 通过对话保存研究立场和验证条件、维护自选、记录已发生的买卖；成交记录支持幂等重试，持仓与现金由账本推导；
- 通过对话创建和修改个人页面，组合持仓、研究立场、自选与研究记录组件；页面保留版本，打开或刷新时读取当前数据；
...
- 行情按来源优先级依次尝试（腾讯 → 新浪 → Alpaca），第一个可用的来源即被采用，单次尝试超过 2 秒就降级到下一个来源，不会等待最慢的备用来源；
- 组合页展示行情观测时间与来源，并提供「刷新行情」按钮：只重取行情、不重新加载页面；同一批行情在 `MARKET_PRICE_CACHE_TTL_MS`（默认 10 秒）内由组合与盈利保护共用一次取价，手动刷新会绕过该缓存；
- 组合权益历史只在全部持仓行情可用时保存，并从启用该能力后开始积累；
- 盈利保护计划按版本保存基准价、失效价、核心仓比例、最大仓位和可选财报风险窗口；基准价默认读取制定时的现价（无可用行情时回退为平均成本并提示），用户只需决定失效价；2R、3R、4R 阶梯、EMA20 移动保护和利润回吐由普通代码计算；
- Tracking 扫描会更新计划生效后的价格高水位并生成一次性待处理提醒；提醒需要用户确认，系统不会自动减仓；
- 设置 `TRACKING_SCAN_INTERVAL_MINUTES` 可启用实例内部定时扫描，`0` 表示关闭；当前版本仍由组合页面请求生成权益快照。
