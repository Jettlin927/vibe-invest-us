...
function ProfitProtectionDialog({ position, current, onCancel, onSubmit }: {
  ...
  const observedPrice = current?.marketPrice ?? position.marketPrice ?? null
  const autoAnchor = current?.anchorPrice ?? observedPrice ?? position.averageCost
  const anchorOrigin = current
    ? `基准价沿用本版计划冻结值 ${formatNumber(current.anchorPrice)}${observedPrice === null ? '' : `；当前现价 ${formatNumber(observedPrice)}`}，改持仓不会静默改写它。`
    : observedPrice === null
      ? `行情暂不可用，基准价已回退为平均成本 ${formatNumber(position.averageCost)}；取得行情后可重新制定。`
      : `基准价自动读取制定时的现价 ${formatNumber(observedPrice)}，你只需要决定失效价。`
  const [anchorManual, setAnchorManual] = useState(false)
  const [anchorPrice, setAnchorPrice] = useState(String(autoAnchor))
